#!/bin/bash
set -euo pipefail

if [ $EUID -ne 0 ]; then
    echo "Please run this with sudo."
    exit 1
fi

GPGDIR=/tmp/lab7
PRIVKEY="$PWD/lab7privkey"

mkdir -p $GPGDIR

gpg --homedir "$GPGDIR" --import "$PRIVKEY"

decrypt() { gpg --homedir "$GPGDIR" -d --output "$1"; }

chmod 404 file3.txt && chown nobody file3.txt
cat <<EOF | decrypt file3.txt
-----BEGIN PGP MESSAGE-----


=uKIk
-----END PGP MESSAGE-----
EOF

chmod 644 file4.txt && chown nobody file4.txt
cat <<EOF | decrypt file4.txt
-----BEGIN PGP MESSAGE-----


=ui/g
-----END PGP MESSAGE-----
EOF

chmod 400 file6.txt && chown nobody file6.txt
cat <<EOF | decrypt file6.txt
-----BEGIN PGP MESSAGE-----


=Nxi7
-----END PGP MESSAGE-----
EOF

chmod 400 file7.txt && chown nobody file7.txt
cat <<EOF | decrypt file7.txt
-----BEGIN PGP MESSAGE-----


=by0k
-----END PGP MESSAGE-----
EOF

chmod 777 file8.txt && chown nobody file8.txt
cat <<EOF | decrypt file8.txt
-----BEGIN PGP MESSAGE-----


=DrLZ
-----END PGP MESSAGE-----
EOF

chmod 777 file9.txt && chown nobody file9.txt
cat <<EOF | decrypt file9.txt
-----BEGIN PGP MESSAGE-----


G3UULWemw6cKfk6Cv+Kw
=kUgM
-----END PGP MESSAGE-----
EOF
